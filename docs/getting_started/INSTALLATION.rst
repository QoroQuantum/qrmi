...
Maestro Local is available in the QoroQuantum fork. Install QRMI from that
checkout using the source installation instructions above; no optional Python
extra is needed for Maestro. Run the matching native Maestro local server and
select its Unix socket with ``QRMI_MAESTRO_SOCKET`` when it is not
``/run/maestro.sock``. The server and its workers use Rust/C/C++.

See :doc:`../maestro-native-api` for native requests, noise simulation and
distribution, :doc:`../migration/maestro-0.24` for discovery and rebuilding
bindings, and :doc:`../examples/task_runner/TASK_RUNNER_MAESTRO` for examples.
