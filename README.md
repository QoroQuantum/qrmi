...
> [!NOTE]
> Maestro Local requires no optional extra and connects to a locally running Maestro server over a Unix domain socket. Install QRMI from this fork to use it. The [Maestro Local examples](examples/qiskit_primitives/maestro_local) additionally require `qiskit` to build circuits.
> See the [native request guide](docs/maestro-native-api.rst) for noise simulation, state queries and GPU distribution, and the [Maestro migration notes](docs/migration/maestro-0.24.rst) for service discovery, session/error behavior, and rebuilding bindings after an upstream merge.
